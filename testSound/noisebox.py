...
DEBUG = True  # Change this to `True` to print what's played
...
SOUNDS = {}
with open(os.path.join(APP_PATH, "mix2.json")) as j:
    MIX = json.load(j)
...
def do_random_pick():
    time.sleep(START_ON/100)
    play_random_pick()
    while True:
        next_play = random.randrange(SECONDS_MIN, SECONDS_MAX)/100
